...
resource "azurerm_network_interface" "tfazapp2_nic" {
  count               = 2
  name                = "tfazapp2-vm1-nic_${count.index}"
...
resource "azurerm_linux_virtual_machine" "tfazapp2_linux_vm" {
  name                  = "tfazapp2vm"
  location              = var.location
  resource_group_name   = var.rg-name
...
resource "azurerm_virtual_machine" "tfazapp2_vm" {
  ...
  location              = var.location
  resource_group_name   = var.rg-name
  availability_set_id   = azurerm_availability_set.tfazapp2_aset 
...
resource "azurerm_virtual_machine_data_disk_attachment" "tfazapp2_vmdisk_attach" {
  managed_disk_id    = element(azurerm_managed_disk.tfazapp2_disk.*.id, count.index)
  virtual_machine_id = element(azurerm_virtual_machine.tfazapp2_vm.*.id, count.index)
  lun                = "1"
  caching            = "ReadWrite"
}